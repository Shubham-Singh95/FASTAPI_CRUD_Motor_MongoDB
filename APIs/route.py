...
from APIs.repository import BookRepo
from Mongo_DB.models import Book, Response
...
async def create(book: Book):
    ...
    return Response(code=201, status="Ok", message="Success save data").dict(exclude_none=True)
...
async def update(book: Book):
    ...
    return Response(code=201, status="Ok", message="Success update data").dict(exclude_none=True)
